...
class AdminController extends Controller
{
     ...
     */
     ...
     public function workunitList(Request $request)
     {
        $search = $request['search'] ?? "";

        if($search != "") {

            $workunits = WorkUnit::where(function ($query) use($search) {
                    $query->where('name', 'LIKE', "%{$search}%")
                        ->where('school_id', auth()->user()->school_id);
                })->paginate(10);

        } else {
            $workunits = WorkUnit::where('school_id', auth()->user()->school_id)->paginate(10);
        }

        return view('admin.workunit.workunit_list', compact('workunits', 'search'));
     }


     public function createWorkUnit()
     {
         return view('admin.workunit.add_workunit');
     }
     ...
     public function workUnitCreate(Request $request)
     {
         ...
         $duplicate_workunit_check = Workunit::get()->where('name', $data['name'])->where('school_id', auth()->user()->school_id);
 
         if(count($duplicate_workunit_check) == 0) {
            Workunit::create([
                 'name' => $data['name'],
                 'school_id' => auth()->user()->school_id,
             ]);
 
             return redirect()->back()->with('message','Tạo đơn vị thành công');
 
         } else {
             return back()
             ->with('error','Xin lỗi đơn vị đã tồn tại trong cơ sở dữ liệu');
         }
     }
     ...
     public function editWorkUnit($id)
     {
         ...
         return view('admin.workunit.edit_workunit', ['workunit' => $workunit]);
     }
 
     public function workunitUpdate(Request $request, $id)
     {
         $data = $request->all();
     ...
     public function workunitDelete($id)
     {
         $workunit = Workunit::find($id);
         $workunit->delete();
    ...
    public function classCreate(Request $request)
    {
        ...
        $duplicate_class_check = Classes::get()->where('code', $data['code'])->where('school_id', auth()->user()->school_id);
    ...
    public function classUpdate(Request $request, $id)
    {
        ...
        if($data['code'] != null) {
            Classes::where('id', $id)->update([
                ...
                'end_date'   => strtotime($data['end_date'] .' 00:00:00'),
                'teacher_id' => $data['teacher_id'],
                'time_hours' => $data['time_hours'],
                'class_room_id' => $data['class_room_id'],
                'work_unit_id' => $data['work_unit_id'],
            ...
            return redirect()->back()->with('message','Đã cập nhật thông tin lớp học.');
        } else {
            return back()
            ->with('error','Mã lớp học không được trống');
        }
    }
    ...
    public function createExpense()
    {
        $expense_categories = ExpenseCategory::where('school_id', auth()->user()->school_id)->get();
        $classes = Classes::where('school_id', auth()->user()->school_id)->get();
        return view('admin.expenses.create', ['expense_categories' => $expense_categories , 'classes' => $classes]);
    }
